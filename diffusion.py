...
class diffusionRecipe(A.recipe):
    ...
    def cell_description(self, _):
        ...
        sm = tree.append(A.mnpos,
                         (0, 0,                0, self.radius_soma),
                         (0, self.length_soma, 0, self.radius_soma),
                         tag=tag_soma)
        _ = tree.append(sm,
                        (0, 0,                    0, self.radius_dendrite),
                        (0, self.length_dendrite, 0, self.radius_dendrite),
                        tag=tag_dendrite1)
        _ = tree.append(sm,
                        (0, 0,                    0, self.radius_dendrite),
                        (0, self.length_dendrite, 0, self.radius_dendrite),
...
def run_sim(points, r_soma, r_dend, tx):
    ...
    hdls = [sim.sample((0, i), A.regular_schedule(0.1*dt_)) for i,_ in enumerate(rec.probes(0))]
    ...
    ax = axs[1][2]
    for data, meta in sim.samples(hdls[14]):
            ax.plot(data[:, 0], data[:, 1], ls=':')
    ...
    # total injected
    N = rec.ch_0*len(rec.times_0) + rec.ch_1*len(rec.times_1) + rec.ch_2*len(rec.times_2)
    # equilibrium concentration
    X = N/rec.volume
    # per CV count = concentration * CV_volume
    n_soma = X*rec.delta_x*np.pi*rec.radius_soma**2
    n_dend = X*rec.delta_x*np.pi*rec.radius_dendrite**2
    axs[0][0].hlines(X, 0, 5, color='0.8')
    axs[0][1].hlines(X, 0, 5, color='0.8')
    axs[0][2].hlines(X, 0, 5, color='0.8')
    axs[1][0].hlines(n_soma, 0, 5, color='0.8')
    axs[1][1].hlines(n_dend, 0, 5, color='0.8')
    axs[1][2].hlines(n_dend, 0, 5, color='0.8')
